...
module.exports = async function branchDone(type) {
  ...
  await execa('git', ['checkout', 'develop']).catch(e => console.log(chalk.red(e)))
  ...
  if (!isConfirm) {
    await execa('git', ['reset', '--hard', 'HEAD~']).catch(e => console.log(chalk.red(e)))
    return
  }
  await execa('npm', ['version', type]).catch(e => console.log(chalk.red(e)))
}