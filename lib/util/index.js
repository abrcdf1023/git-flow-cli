...
module.exports.startCommit = async function startCommit(type, workingBranch) {
  const commitMsg = await inquirer.prompt([{
    name: 'commitMsg',
    message: 'Please commit...'
  }]).then((answers) => answers.commitMsg)

  if (!type) {await execa('git', ['commit', '-m', commitMsg])}
  else if (type && workingBranch) {
    await checkUpdateVersion(type, workingBranch)
    startSpinner('Updating version please wait...')
    await execa('npm', ['version', type, '-m', commitMsg])
    stopSpinner('succeed', 'Update succeed.')
  }
  stopSpinner('succeed', 'Commit succeed.')
}

module.exports.fetchRemoteBranch = async function fetchRemoteBranch() {
