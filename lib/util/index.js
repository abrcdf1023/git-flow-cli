...
module.exports.mergeBranch = async function mergeBranch(workingBranch, checkoutBranch) {
  ...
  let mergeInfo = await execa('git', ['merge', workingBranch])
  ...
  } else if (mergeInfo.includes('Already up to date.')) {
 ...
 * @param {string} branchname the branch want to delete
 */
module.exports.deleteLocalBranch = async function deleteLocalBranch(branchname) {
  startSpinner('Deleting branch please wait...')
  await execa('git', ['branch', '-d', branchname])
  stopSpinner('succeed', 'Delete succeed.')
}
